...
function ActionInner(props) {
  ...
  return (
    <span onClick={actionAlert} style={{ display: "block", color: "red" }}>
      {props.text}
    </span>
  );
}
...
export function EmptyTemplate(props) {
  return (
    <section style={{ textAlign: "center" }}>
      {props.text}
      <span className="plugin-action" onClick={props.testFN}>
        state
      </span>
    </section>
  );
...
export function TitleComponents(props) {
  return (
    <a
      href={
        "https://www.lovejavascript.com/#!zone/blog/content.html?id=" +
        props.row.id
      }
      target={"_black"}
    >
...
export function TypeComponents(props) {
  ...
  return <button>{TYPE_MAP[props.type]}</button>;
}
// 组件: 删除
