...
const Table = React.forwardRef((props: TableProps, ref) => {
  const {
    ...
    headerHeight: headerHeightProp = 35,
    minHeight = 0,
    height = 200,
    autoHeight,
    fillHeight,
  ...
  const [data, setData] = useState(() => {
    return isTree
      ? filterTreeData(dataProp, expandedRowKeys, rowKey)
      : dataProp;
  });
  ...
  const shouldFixedColumn = Array.from(
    flatten(children as any) as Iterable<any>
  ).some((child) => child?.props?.fixed);

  // Check all column properties for the existence of rowSpan.
  const shouldRowSpanColumn = Array.from(
    flatten(children as any) as Iterable<any>
  ).some((child) => child?.props?.rowSpan);
  ...
  const {
    ...
    onTableScroll: debounce(
      (coords: { x?: number; y?: number }) => onScrollTo(coords),
      100
    ),
  ...
  const { headerCells, bodyCells, allColumnsWidth, hasCustomTreeCol } =
    useCellDescriptor({
      children,
      rtl,
      mouseAreaRef,
  ...
  useUpdateEffect(() => {
    setData(
      isTree ? filterTreeData(dataProp, expandedRowKeys, rowKey) : dataProp
    );
  ...
  const rowWidth =
    allColumnsWidth > tableWidth.current ? allColumnsWidth : tableWidth.current;
            ...
            <CellGroup
              ...
              style={
                rtl
                  ? {
                      right:
                        tableWidth.current - fixedLeftCellGroupWidth - rowRight,
                    }
                  : undefined
              }
            >
          ...
          {fixedRightCellGroupWidth ? (
            ...
            >
              {mergeCells(
                resetLeftForCells(
                  fixedRightCells,
                  hasVerticalScrollbar ? SCROLLBAR_WIDTH : 0
                )
              )}
            </CellGroup>
          ) : null}
  ...
  const renderRowData = (
    ...
    const hasChildren =
      isTree && rowData.children && Array.isArray(rowData.children);
    const nextRowKey =
      rowKey && typeof rowData[rowKey] !== "undefined"
        ? rowData[rowKey]
        : props.key;
    ...
    const expanded = expandedRowKeys.some(
      (key) => rowKey && key === rowData[rowKey]
    );
  ...
  const renderTableBody = (bodyCells: any[], rowWidth: number) => {
    ...
    if (data) {
      ...
      const isUncertainHeight =
        !!renderRowExpandedProp || isCustomRowHeight || wordWrap;
      ...
      if ((isUncertainHeight && virtualized) || !virtualized) {
        ...
        for (let index = 0; index < data.length; index++) {
          ...
          visibleRows.current.push(
            renderRowData(bodyCells, rowData, rowProps, shouldRender)
          );
        }
      } else {
        /** virtualized */
        ...
        for (let index = startIndex; index < endIndex; index++) {
          ...
          visibleRows.current.push(
            renderRowData(bodyCells, rowData, rowProps, false)
          );
        }
      }
    }
    ...
    return (
      ...
      >
        ...
        >
          {topHideHeight ? (
            <Row style={topRowStyles} className="virtualized" />
          ) : null}
          {visibleRows.current}
          {bottomHideHeight ? (
            <Row style={bottomRowStyles} className="virtualized" />
          ) : null}
        </div>
...
Table.propTypes = {
  ...
  affixHorizontalScrollbar: PropTypes.oneOfType([
    PropTypes.bool,
    PropTypes.number,
  ]),
