...
export interface InnerCellProps extends Omit<CellProps, "children"> {
  ...
  children?:
    | React.ReactNode
    | ((rowData: RowDataType, rowIndex?: number) => React.ReactNode);
...
const Cell = React.forwardRef(
  (props: InnerCellProps, ref: React.Ref<HTMLDivElement>) => {
    const {
      classPrefix = "cell",
      width = 0,
      left = 0,
    ...
    const contentStyles: React.CSSProperties = {
      ...
      height: nextHeight,
      textAlign: align,
      [paddingKey]: isTreeCol
        ? depth * LAYER_WIDTH + 10
        : style?.[paddingKey] || style?.padding,
    };
    ...
    if (wordWrap) {
      contentStyles.wordBreak =
        typeof wordWrap === "boolean" ? "break-all" : wordWrap;
      contentStyles.overflowWrap =
        wordWrap === "break-word" ? wordWrap : undefined;
    }
    ...
    const renderTreeNodeExpandIcon = () => {
      ...
      const expandButton = (
        <ExpandIconComponent className={prefix("expand-icon")} />
      );
