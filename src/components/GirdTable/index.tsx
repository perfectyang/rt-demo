...
export default class ReactGridManager extends React.Component {
   ...
   */
  toReact(compileList, isAdd = false) {
  ...
  updateReactTemplate() {
    ...
    let {
      columnData,
      emptyTemplate = settings.emptyTemplate,
      fullColumn = settings.fullColumn,
    } = $gridManager.updateTemplate(this.option);
    ...
    // 更新已缓存的模板
    this.reactCache.forEach((item: any) => {
      switch (item.type) {
  ...
  updateClassName() {
    ...
    const classList = document.querySelector(
      `[grid-manager-wrap="${gridManagerName}"]`
    ).classList;
...
const noExtendsList = ["name", "length", "prototype"];
