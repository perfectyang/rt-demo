import { InputTag, Tag, Tooltip, InputTagProps } from '@arco-design/web-react';
import React, { useCallback, useEffect } from 'react';
import { useMemo } from 'react';
import './tags.css'


interface ILabel {label: string, value: string}
interface IProps {
...
const RenderTag: React.FC<IProps> = ({value, onChange}) => {
  ...
  const texts = useMemo(() => {
    return tagList.reduce((pre, next) => ([...pre, next.label]), [] as string[]).join(',')
  }, [tagList])


  return (
    <div className='tag-wrap' ref={tagRef} onMouseOver={() => {
